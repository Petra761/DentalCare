...
export const apiService = {
  ...
  async login(nombreUsuario: string, contrasena: string) {
    if (isMockMode) {
      ...
      const user = users.find(
        (u) =>
          u.nombreUsuario.toLowerCase() === nombreUsuario.toLowerCase() &&
      ...
      return {
        ...
        usuario: {
          ...
          rol:
            user.rol ||
            MOCK_ROLES.find((r) => r.id === user.idRol)?.nombre ||
            "Paciente",
          estado: user.estado,
        },
      };
    ...
    if (!res.ok) {
      const errData = await res
        .json()
        .catch(() => ({ mensaje: "Error al iniciar sesión" }));
  ...
  /*dashboard clientes registrados en el sistema*/
  // Solo para Dashboard
  async getClientesDashboard(): Promise<any> {
    ...
    if (isMockMode) {
      ...
      const defaultMock: Servicio[] = [
        ...
        },
        {
          ...
          descripcion: "Control de brackets",
          duracion: "00:30:00",
          estadoServicio: "Disponible",
          estado: "Activo",
        },
        {
          ...
          idCategoria: 3,
          codigo: "SRV003",
          nombre: "Implante Fase 2",
          descripcion: "Fase 2 de implantes",
          duracion: "01:30:00",
          estadoServicio: "Disponible",
          estado: "Activo",
        },
  ...
  async getDetallesCita(): Promise<DetalleCita[]> {
    ...
    const res = await fetch(`${BASE_URL}/DetallesCita`, {
      headers: getHeaders(),
    });
  ...
  async crearNuevaCita(dto: NuevaCitaDto): Promise<DbCita> {
    ...
    if (!res.ok) {
      const errData = await res
        .json()
        .catch(() => ({ mensaje: "Error al registrar la cita" }));
      throw new Error(
        errData.mensaje || "Error al agendar la cita en el servidor",
      );
    }
  ...
  async actualizarEstadoCita(
    ...
    if (!res.ok) {
      const errData = await res
        .json()
        .catch(() => ({ mensaje: "Error al actualizar el estado" }));
      throw new Error(
        errData.mensaje || "Error al actualizar el estado de la cita",
      );
    }
  },
};
